...
object TraverseLaws {
  ...
  */
  ...
  fun <F, A> laws(TF: Traverse<F>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, MA: Monoid<A>, EQA: Eq<A>, EQ: Eq<Kind<F, A>>) =
    laws(TF, arbF, arbA, functionAToBArbitrary(arbA), MA, EQA, EQ)

  fun <F, A> laws(TF: Traverse<F>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, arbAToA: Arbitrary<(A) -> A>, MA: Monoid<A>, EQA: Eq<A>, EQ: Eq<Kind<F, A>>) =
    laws(TF, arbF, arbA, arbAToA, arbAToA, MA, EQA, EQ)

  fun <F, A, B> laws(TF: Traverse<F>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, arbAToB: Arbitrary<(A) -> B>, arbBToA: Arbitrary<(B) -> A>, MA: Monoid<A>, EQA: Eq<A>, EQ: Eq<Kind<F, A>>): List<Law> =
    FoldableLaws.laws(TF, arbF, makeFunAtoA(arbAToB, arbBToA), MA, EQA) + FunctorLaws.laws(TF, arbF, arbAToB, arbBToA, EQ) + listOf(
      Law("Traverse Laws: Identity", TF.identityTraverse(arbF, arbA, EQ)),
  ...
  fun <F, A> Traverse<F>.identityTraverse(arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, EQ: Eq<Kind<F, A>>) = Id.applicative().run {
    val idApp = this
    forAll(Tuple2.arbitrary(
      ...
      fa.traverse(idApp, f).extract()
        .eqv(fa.map(f).map { it.extract() }, EQ)
    }
  }
