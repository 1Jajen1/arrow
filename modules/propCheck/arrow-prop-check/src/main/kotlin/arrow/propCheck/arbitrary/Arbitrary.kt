...
fun arbitrarySizedShort(): Gen<Short> = Gen.sized { n ->
  Gen.choose(
    when {
      -n < Short.MIN_VALUE -> Short.MIN_VALUE
      else -> (-n).toShort()
    } toT when {
      n > Short.MAX_VALUE -> Short.MAX_VALUE
      else -> n.toShort()
    },
    Short.random()
  )
...
fun arbitraryBoundedShort(): Gen<Short> = Gen.chooseAny(Short.random())
