...
import arrow.test.generators.ThrowableArbitrary
...
object ConcurrentLaws {
  ...
  fun <F> laws(AP: Concurrent<F>, EQ_EITHER: Eq<Kind<F, Either<Throwable, Int>>>, EQ: Eq<Kind<F, Int>>, EQ_UNIT: Eq<Kind<F, Unit>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true, ctx: CoroutineContext = Dispatchers.Default): List<Law> = laws(
    AP,
    Int.arbitrary(),
    Int.monoid(),
  ...
  fun <F, A> laws(AP: Concurrent<F>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<Throwable, A>>>, EQ: Eq<Kind<F, A>>, EQ_UNIT: Eq<Kind<F, Unit>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true, ctx: CoroutineContext = Dispatchers.Default) =
    laws(AP, applicativeErrorArbitrary(arbA, NonFatalThrowableArbitrary, AP), arbA, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ, EQ_UNIT, unsafeRun, testStackSafety, ctx)

  fun <F, A> laws(AP: Concurrent<F>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<Throwable, A>>>, EQ: Eq<Kind<F, A>>, EQ_UNIT: Eq<Kind<F, Unit>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true, ctx: CoroutineContext = Dispatchers.Default) =
    laws(AP, arbF, arbA, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ, EQ_UNIT, unsafeRun, testStackSafety, ctx)

  fun <F, A, B> laws(AP: Concurrent<F>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoB: Arbitrary<(A) -> B>, arbBtoA: Arbitrary<(B) -> A>, EQ_EITHER: Eq<Kind<F, Either<Throwable, A>>>, EQ: Eq<Kind<F, A>>, EQ_UNIT: Eq<Kind<F, Unit>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true, ctx: CoroutineContext = Dispatchers.Default) =
    laws(AP, applicativeErrorArbitrary(arbA, NonFatalThrowableArbitrary, AP), arbA, MA, arbAtoB, arbBtoA, EQ_EITHER, EQ, EQ_UNIT, unsafeRun, testStackSafety, ctx)
