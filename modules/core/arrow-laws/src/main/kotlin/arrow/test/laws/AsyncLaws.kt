...
object AsyncLaws {
  ...
  fun <F> laws(AP: Async<F>, EQ_EITHER: Eq<Kind<F, Either<Throwable, Int>>>, EQ: Eq<Kind<F, Int>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true): List<Law> = laws(
    AP,
    applicativeErrorArbitrary(Int.arbitrary(), NonFatalThrowableArbitrary, AP),
    Int.arbitrary(),
    Int.monoid(),
  ...
  fun <F, A> laws(AP: Async<F>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<Throwable, A>>>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true) =
    laws(AP, applicativeErrorArbitrary(arbA, NonFatalThrowableArbitrary, AP), arbA, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ, unsafeRun, testStackSafety)

  fun <F, A> laws(AP: Async<F>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<Throwable, A>>>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true) =
    laws(AP, arbF, arbA, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ, unsafeRun, testStackSafety)

  fun <F, A, B> laws(AP: Async<F>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoB: Arbitrary<(A) -> B>, arbBtoA: Arbitrary<(B) -> A>, EQ_EITHER: Eq<Kind<F, Either<Throwable, A>>>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int, testStackSafety: Boolean = true) =
    laws(AP, applicativeErrorArbitrary(arbA, NonFatalThrowableArbitrary, AP), arbA, MA, arbAtoB, arbBtoA, EQ_EITHER, EQ, unsafeRun, testStackSafety)
