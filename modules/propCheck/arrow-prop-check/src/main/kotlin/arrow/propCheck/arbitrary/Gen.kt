...
interface GenMonad : Monad<ForGen> {
  ...
  private tailrec fun <A, B> step(r: RandSeed, size: Int, a: A, f: (A) -> GenOf<Either<A, B>>): B {
    val (r1, r2) = r.split()
    val res = f(a).fix().unGen(r1 toT size)
    return when (res) {
      is Either.Left -> step(r2, size, res.a, f)
      is Either.Right -> res.b
    }
  }

  override fun <A, B> tailRecM(a: A, f: (A) -> Kind<ForGen, Either<A, B>>): Kind<ForGen, B> =
    Gen { (r, s) -> step(r, s, a, f) }
}