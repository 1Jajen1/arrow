...
import arrow.propCheck.rose.monad.monad
...
interface RoseMonad : Monad<ForRose> {
  ...
  override fun <A, B> tailRecM(a: A, f: (A) -> Kind<ForRose, Either<A, B>>): Kind<ForRose, B> =
    f(a).fix().flatMap { either ->
      either.fold({ tailRecM(it, f) }, ::just).fix()
    }
}
...
fun <A, B> forAllShrink(
  ...
  forAllShrinkShow(genB, shrinkerB, testA, { showB.run { it.show() } }, prop)
...
@JvmName("forAll")
inline fun <reified A : Any, B> forAll(
  arbB: Arbitrary<B>,
...
infix fun Property.and(b: Property): Property = this and Eval.now(b)

infix fun Property.and(b: () -> Property): Property = this and Eval.later(b)

infix fun Property.and(b: Eval<Property>): Property =
  conjoin(
    sequenceOf(
      Eval.now(this),
      b
...
infix fun Property.or(b: Property): Property = this or Eval.now(b)

infix fun Property.or(b: () -> Property): Property = this or Eval.later(b)

infix fun Property.or(b: Eval<Property>): Property =
  disjoin(
    sequenceOf(
      Eval.now(this),
      b
