...
object MonadFilterLaws {
  ...
  fun <F> laws(S: MonadFilter<F>, EQ: Eq<Kind<F, Int>>, unsafeRun: Kind<F, Int>.() -> Int): List<Law> = laws(
    S,
    Int.arbitrary(),
    Int.monoid(),
  ...
  fun <F, A> laws(S: MonadFilter<F>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAToA: Arbitrary<(A) -> A>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int) =
    laws(S, applicativeArbitrary(arbA, S), arbA, MA, arbAToA, arbAToA, EQ, unsafeRun)

  fun <F, A> laws(S: MonadFilter<F>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int) =
    laws(S, arbF, arbA, MA, arbAtoA, arbAtoA, EQ, unsafeRun)

  fun <F, A, B> laws(S: MonadFilter<F>, arbA: Arbitrary<A>, MA: Monoid<A>, arbAToB: Arbitrary<(A) -> B>, arbBToA: Arbitrary<(B) -> A>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int) =
