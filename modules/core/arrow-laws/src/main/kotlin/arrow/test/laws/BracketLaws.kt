...
object BracketLaws {
  ...
  fun <F> laws(AP: Bracket<F, Throwable>, EQ_EITHER: Eq<Kind<F, Either<Throwable, Int>>>, EQ: Eq<Kind<F, Int>>, unsafeRun: Kind<F, Int>.() -> Int): List<Law> = laws(
    AP,
    NonFatalThrowableArbitrary,
    EQ_EITHER, EQ, unsafeRun
  )

  fun <F, E> laws(AP: Bracket<F, E>, arbE: Arbitrary<E>, EQ_EITHER: Eq<Kind<F, Either<E, Int>>>, EQ: Eq<Kind<F, Int>>, unsafeRun: Kind<F, Int>.() -> Int): List<Law> = laws(
    AP,
  ...
  fun <F, A, E> laws(AP: Bracket<F, E>, arbA: Arbitrary<A>, arbE: Arbitrary<E>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<E, A>>>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int) =
    MonadErrorLaws.laws(AP, applicativeErrorArbitrary(arbA, arbE, AP), arbA, arbE, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ, unsafeRun)

  fun <F, A, E> laws(AP: Bracket<F, E>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, arbE: Arbitrary<E>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<E, A>>>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int) =
    MonadErrorLaws.laws(AP, arbF, arbA, arbE, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ, unsafeRun)

  fun <F, A, B, E> laws(AP: Bracket<F, E>, arbA: Arbitrary<A>, arbE: Arbitrary<E>, MA: Monoid<A>, arbAtoB: Arbitrary<(A) -> B>, arbBtoA: Arbitrary<(B) -> A>, EQ_EITHER: Eq<Kind<F, Either<E, A>>>, EQ: Eq<Kind<F, A>>, unsafeRun: Kind<F, Int>.() -> Int) =
    MonadErrorLaws.laws(AP, applicativeErrorArbitrary(arbA, arbE, AP), arbA, arbE, MA, arbAtoB, arbBtoA, EQ_EITHER, EQ, unsafeRun)
