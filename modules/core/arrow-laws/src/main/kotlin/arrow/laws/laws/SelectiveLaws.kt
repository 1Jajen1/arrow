...
object SelectiveLaws {
  ...
  fun <F> laws(S: Selective<F>, EQ: Eq<Kind<F, Int>>): List<Law> = laws(
    S, Int.arbitrary(), Int.monoid(), EQ
  )

  fun <F, A> laws(S: Selective<F>, arbA: Arbitrary<A>, MA: Monoid<A>, EQ: Eq<Kind<F, A>>): List<Law> =
    laws(S, arbA, MA, functionAToBArbitrary(arbA), EQ)
