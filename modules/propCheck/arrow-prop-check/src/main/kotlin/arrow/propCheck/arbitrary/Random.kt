...
interface ShortRandom : Random<Short> {
  override fun random(seed: RandSeed): Tuple2<Short, RandSeed> =
    seed.nextInt(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt()).let { (a, s) ->
      a.toShort() toT s
    }

  override fun randomR(range: Tuple2<Short, Short>, seed: RandSeed): Tuple2<Short, RandSeed> =
    if (range.a == range.b) range.a toT seed
    else seed.nextInt(range.a.toInt(), range.b.toInt()).let { (a, s) ->
      a.toShort() toT s
    }
...
fun Short.Companion.random(): Random<Short> = object : ShortRandom {}
