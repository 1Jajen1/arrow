...
import arrow.propCheck.arbitrary.RandSeed
...
internal fun addDiscarded(discard: Int): String =
  (if (discard > 0)
    " discarded $discard"
  else String.monoid().empty()) // Tbh this is just cheating detekt which complains about duplicate "" strings

internal fun afterNumTests(tests: Int): String =
  "after " + tests.number("test")
...
internal fun failureMessage(result: Result): String = when (result) {
  is Result.Failure -> "Failed: " + result.reason +
    ...
    afterNumTests(result.numTests) +
    addDiscarded(result.numDiscardedTests) +
    (if (result.numShrinks > 0)
      " shrunk " + result.numShrinks.number("time")
    else "") + usedSeed(result.usedSeed, result.usedSize)
  is Result.GivenUp -> "Gave up " + afterNumTests(result.numTests) +
    addDiscarded(result.numDiscardedTests)
  is Result.NoExpectedFailure -> "No expected failure " + afterNumTests(result.numTests) +
    addDiscarded(result.numDiscardedTests)
...
private const val dLow = 0.02425
