...
object ApplicativeErrorLaws {
  ...
  fun <F> laws(AE: ApplicativeError<F, Throwable>, EQ_EITHER: Eq<Kind<F, Either<Throwable, Int>>>, EQ: Eq<Kind<F, Int>>): List<Law> = laws(
    AE,
    NonFatalThrowableArbitrary,
    EQ_EITHER, EQ
  )

  fun <F, E> laws(AE: ApplicativeError<F, E>, arbE: Arbitrary<E>, EQ_EITHER: Eq<Kind<F, Either<E, Int>>>, EQ: Eq<Kind<F, Int>>): List<Law> = laws(
    AE,
    applicativeErrorArbitrary(Int.arbitrary(), arbE, AE),
    Int.arbitrary(),
    arbE,
    Int.monoid(),
  ...
  fun <F, A, E> laws(AE: ApplicativeError<F, E>, arbA: Arbitrary<A>, arbE: Arbitrary<E>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<E, A>>>, EQ: Eq<Kind<F, A>>) =
    laws(AE, applicativeErrorArbitrary(arbA, arbE, AE), arbA, arbE, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ)

  fun <F, A, E> laws(AE: ApplicativeError<F, E>, arbF: Arbitrary<Kind<F, A>>, arbA: Arbitrary<A>, arbE: Arbitrary<E>, MA: Monoid<A>, arbAtoA: Arbitrary<(A) -> A>, EQ_EITHER: Eq<Kind<F, Either<E, A>>>, EQ: Eq<Kind<F, A>>) =
    laws(AE, arbF, arbA, arbE, MA, arbAtoA, arbAtoA, EQ_EITHER, EQ)

  fun <F, A, B, E> laws(AE: ApplicativeError<F, E>, arbA: Arbitrary<A>, arbE: Arbitrary<E>, MA: Monoid<A>, arbAtoB: Arbitrary<(A) -> B>, arbBtoA: Arbitrary<(B) -> A>, EQ_EITHER: Eq<Kind<F, Either<E, A>>>, EQ: Eq<Kind<F, A>>) =
    laws(AE, applicativeErrorArbitrary(arbA, arbE, AE), arbA, arbE, MA, arbAtoB, arbBtoA, EQ_EITHER, EQ)
